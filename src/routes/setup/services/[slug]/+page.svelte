<script>
    ...
    import { Database, Scissors } from "@lucide/svelte";
    ...
    const updateService = async (key) => {
        const { error } = await supabase.from('kb_services')
...
<div class="fl-service wrapper">
    ...
    <section class="fl-page-header">
        <div class="name">
            <Scissors size={24} />
            <!-- <FieldText class="name" bind:value={service.name} onblur={() => updateService('name')} /> -->
            <input type="text" class="name" bind:value={service.name} onblur={() => updateService('name')} />
        </div>
        <textarea bind:value={service.description} onblur={() => updateService('description')}></textarea>
    </section>
    ...
    <div class="fl-page-details">
        ...
        <Card class="fl-settings mb">
            <!-- <div class="fl-toggle"> -->
                <Toggle id="online-{service.id}" label="This service can be booked online."
                    bind:checked={service.is_online}
                    ontoggle={() => updateService('is_online')}
                />
